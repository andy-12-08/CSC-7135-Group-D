...
if(isset($_POST["admin_email_address"]))
{
	...
	$url = '';
	$data = array(
		':admin_email_address'	=>	trim($_POST["admin_email_address"])
	);
	...
	if($total_row == 0)
	{
		$object->query = "
			...
			WHERE trim(tutor_email_address) = :admin_email_address
		";
		$object->execute($data);
		...
		else
		{
			...
			{
				if($row["tutor_status"] == 'Inactive')
				{
					$error = '<div class="alert alert-danger">Your account is Inactive, Contact Admin</div>';
				}
				else
				{
					if($_POST["admin_password"] == $row["tutor_password"])
					{
						$_SESSION['admin_id'] = $row['tutor_id'];
						$_SESSION['type'] = 'Doctor';
						$url = $object->base_url . 'admin/tutor_schedule.php';
					}
					else
					{
