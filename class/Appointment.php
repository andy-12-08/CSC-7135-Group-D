...
class Appointment
{
	...
	{
		$this->connect = new PDO("mysql:host=ec2-54-196-156-86.compute-1.amazonaws.com;dbname=tutor_online", "root", "SEgroupd");
	...
	function get_total_today_appointment()
	{
		...
		WHERE tutor_schedule_date = CURDATE() 
		";
		$this->execute();
	...
	function get_total_yesterday_appointment()
	{
		...
		INNER JOIN tutor_schedule_table 
		ON tutor_schedule_table.tutor_schedule_id = appointment_table.tutor_schedule_id 
		WHERE tutor_schedule_date = CURDATE() - 1
		";
		$this->execute();
		return $this->row_count();
	}
	...
	function get_total_seven_day_appointment()
	{
		...
		INNER JOIN tutor_schedule_table 
		ON tutor_schedule_table.tutor_schedule_id = appointment_table.tutor_schedule_id 
		WHERE tutor_schedule_date >= DATE(NOW()) - INTERVAL 7 DAY
		";
		$this->execute();
		return $this->row_count();
	}
